      ...
      {open && (
        <div className="card mb-3 w-80">
          <div ref={boxRef} style={{maxHeight:280, overflowY:'auto'}} className="space-y-2 p-2">
            {messages.map((m,i)=>(<div key={i} className={m.from==='user'?'text-right':'text-left'}><div className="inline-block p-2 rounded-md" style={{background: m.from==='user' ? '#f3f4f6' : 'white'}}>{m.text}</div></div>))}
          </div>
